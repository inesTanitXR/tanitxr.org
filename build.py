...
DONATE_URL = "https://donors.tuesday.app/campaign/CMLPDTO"
DONATE_URL_SUPPORT = "https://donors.tuesday.app/campaign/CMLPDTO"
...
.cb-art img.cb-inset{position:absolute;inset:auto 4% 5% auto;width:40%;height:auto;object-fit:cover;aspect-ratio:8/5;
  border-radius:10px;border:4px solid #fdf8f0;box-shadow:0 14px 34px rgba(60,40,26,.25)}
...
#walk-stage{position:fixed;inset:0;z-index:0;cursor:grab;touch-action:pan-y;
  background:radial-gradient(125% 95% at 50% 34%,#fdf8ef 0%,#f2e9da 46%,#e2d4bd 100%)}
...
.rc-ring{fill:none;stroke:#8a6a4d;stroke-width:2;stroke-dasharray:7 11;opacity:.5}
.rc-arrow{fill:none;stroke:#a35f3f;stroke-width:5;stroke-linecap:round}
.rc-head{fill:#a35f3f}
...
#walk-label>*{pointer-events:auto}
.wf-round{position:fixed;top:50%;transform:translateY(-50%);width:44px;height:44px;
  border-radius:50%;border:1px solid rgba(74,53,43,.2);background:rgba(253,248,240,.86);
  color:#4a3527;font-size:22px;line-height:1;cursor:pointer;display:flex;align-items:center;
  justify-content:center;transition:.2s}
.wf-round:hover{background:#4a3527;color:#fdf8f0;border-color:#4a3527}
.wf-l{inset-inline-start:26px}
.wf-r{inset-inline-end:26px}
.wf-panel{position:fixed;z-index:5;inset-inline-start:34px;bottom:34px;max-width:min(430px,calc(100vw - 68px))}
.wf-id{color:#8a735c;font-size:11px;letter-spacing:.14em;text-transform:uppercase}
#walk-label h2{font-family:var(--serif);font-size:clamp(26px,3.6vw,40px);color:#2e2118;
  font-weight:400;margin:7px 0 15px;line-height:1.1}
...
.st-tabs{display:flex;gap:6px;margin-bottom:10px}
.st-tab{flex:1;border:1px solid rgba(74,53,43,.18);background:none;color:#7d6a58;
  border-radius:999px;padding:6px 10px;font:inherit;font-size:12.5px;cursor:pointer}
.st-tab.on{background:#4a3527;color:#fdf8f0;border-color:#4a3527}
#badge-list{display:grid;gap:8px;max-height:46vh;overflow-y:auto}
#badge-list[hidden]{display:none}
.bd{display:flex;gap:10px;align-items:flex-start;padding:8px;border-radius:6px;
  background:rgba(163,95,63,.05)}
.bd.locked{opacity:.5;background:none}
.bd .bi{flex:0 0 auto;width:34px;height:34px;border-radius:50%;display:flex;align-items:center;
  justify-content:center;font-size:17px;background:var(--gold);color:#241a10}
...
#badge-toast .bt-icon{width:34px;height:34px;border-radius:50%;background:var(--gold);
  color:#241a10;display:flex;align-items:center;justify-content:center;font-size:18px}
#badge-toast b{font-size:11px;letter-spacing:.16em;text-transform:uppercase;color:var(--gold)}
#badge-toast p{margin:2px 0 0;font-size:14.5px;font-family:var(--serif)}
#badge-toast .wf-btn{background:none;border-color:rgba(253,248,240,.35);color:#fdf8f0;
  padding:7px 14px;font-size:12.5px;white-space:nowrap}
#badge-toast .wf-btn:hover{background:var(--gold);border-color:var(--gold);color:#241a10}
#bt-close{border:0;background:none;color:rgba(253,248,240,.6);font-size:19px;cursor:pointer}
.st-foot{display:flex;gap:8px;margin-top:12px}
.st-foot .wf-btn{flex:1;padding:9px 10px;font-size:13px}
@media(max-width:760px){
  #saved-chip{top:78px;inset-inline-end:12px;padding:7px 12px;font-size:12.5px}
  #saved-tray{top:126px;inset-inline-end:12px}}
...
.wst{height:105vh;height:105dvh}
...
.wst-intro,.wst-end{min-height:100vh;min-height:100dvh;display:flex;align-items:center;
  justify-content:center;padding:80px 26px;text-align:center}
.warea{height:68vh;height:68dvh;display:flex;align-items:center;justify-content:center;
...
.wcard{max-width:392px;background:rgba(253,249,242,.92);backdrop-filter:blur(10px);
  border:1px solid rgba(74,53,43,.12);border-radius:4px;padding:22px 24px 20px;
  box-shadow:0 20px 56px rgba(60,40,26,.14)}
...
.tsw{border:0;background:none;color:#7d6a58;border-radius:999px;padding:8px 16px;font:inherit;
  font-size:13px;cursor:pointer;display:flex;gap:7px;align-items:center;white-space:nowrap}
.tsw b{color:#a35f3f;font-weight:700}
...
@media(max-width:760px){
  ...
  /* the object's label is a sheet across the bottom here, so a story block centred in the screen
     landed right on top of it. Sit the story above the sheet and let the section grow instead. */
  .warea{height:auto;min-height:68dvh;align-items:flex-start;
...
body.walk-fallback .wst{height:auto}
body.walk-fallback .wsr{position:static;width:auto;height:auto;clip:auto;
  font-family:var(--serif);font-size:20px;font-weight:400;padding:0 26px}
body.walk-fallback .warea{height:auto;padding:36px 26px}
body.walk-fallback .warea>div{opacity:1;transform:none}
@media(max-width:760px){
  .wf-panel{inset-inline-start:16px;bottom:20px;max-width:calc(100vw - 32px)}
  .wf-l{inset-inline-start:12px}.wf-r{inset-inline-end:12px}
  /* on a phone you scroll, so the side arrows only collide with the label */
  .wf-round{display:none}
  .wcard.wide{padding:26px 22px 24px}
}
@media(prefers-reduced-motion:reduce){.warea>div{transition:none;opacity:1;transform:none}}
/* Phones: the object's label is a sheet across the bottom, so a story block centred in the
   screen landed on top of it. Sit the story above the sheet and let the section grow. This
   comes last on purpose: the rules above are declared twice and the later copy wins. */
@media(max-width:760px){
  .warea{height:auto;min-height:68dvh;align-items:flex-start;
    padding-top:calc(96px + env(safe-area-inset-top,0px));
    padding-bottom:calc(252px + env(safe-area-inset-bottom,0px))}
...
def build_walk():
    ...
    for label, sub, preset, artist in rooms:
        ...
        for m in group:
            ...
            i = len(items) - 1
            uid = re.search(r"models/([a-f0-9]+)", m.get("sketchfab") or "")
            fb = ""
            if uid:
    ...
    if os.path.exists(nura_src):
        shutil.copy(nura_src, os.path.join(out_dir, "nura.glb"))
        shipped.add("nura.glb")
...
<ellipse class="rc-ring" cx="120" cy="75" rx="98" ry="34"/>
<path class="rc-arrow" d="M22 75a98 34 0 0 0 40 24"/>
<path class="rc-head" d="M64 88l-14 16 22 4z"/>
<path class="rc-arrow" d="M218 75a98 34 0 0 1-40 24"/>
<path class="rc-head" d="M176 88l14 16-22 4z"/>
</svg>
...
<div id="badge-toast" hidden><span class="bt-icon"></span>
<div><b>Badge earned</b><p id="bt-name"></p></div>
