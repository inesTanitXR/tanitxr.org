...
function start() {
  ...
  function openBubble() {
    ...
    if (speakBtn) speakBtn.title = s.it.voice ? 'اسمع نورا تحكيها' : 'اقرأ هذا بصوت عالٍ';
    ...
    if (moreBtn) {
      moreBtn.textContent = 'أخبريني أكثر';
  ...
  if (moreBtn) moreBtn.addEventListener('click', () => {
    ...
    moreBtn.textContent = expanded ? 'شكرًا يا نورا' : 'أخبريني أكثر';
  ...
  function paintStats(it) {
    ...
    counter('collection_view/' + it.slug).then((v) => {
      ...
      statsEl.textContent = m ? 'شوهدت ' + m.toLocaleString() + '+ مرة' : '';
      statsEl.hidden = !m;
    });
  }
  ...
  function paintMore() {
    ...
    if (snd) snd.textContent = soundOn ? 'الصوت: مشغّل' : 'الصوت: مكتوم';
  ...
  function paintSound() {
    ...
    sndBtn.title = (soundOn ? 'الصوت مشغّل. انقر لكتمه.' : 'الصوت مكتوم. انقر لتشغيله.') + cr;
  }
  paintSound();
  ...
  const BADGES = [
    { id: 'first-turn', icon: '\u21bb', name: 'أول دورة',
      how: 'أدِر قطعة بالمؤشر', test: p => p.rotated >= 1 },
    { id: 'curator', icon: '\u2665', name: 'أمين المجموعة',
      how: 'أحبب خمس قطع', test: p => p.saved.length >= 5 },
    { id: 'listener', icon: '\u25cf', name: 'مستمع جيد',
      how: 'اطلب من نورا المزيد عن عشر قطع', test: p => p.more >= 10 },
    { id: 'surveyor', icon: '\u25c8', name: 'مسّاح المواقع',
      how: 'شاهد قطعة من كل مكان مسحناه',
      test: p => PLACES.every(pl => p.places.includes(pl)) },
    { id: 'guardian', icon: '\u2691', name: 'حارس',
      how: 'شارك قطعة ليراها الآخرون', test: p => p.shared >= 1 },
    { id: 'completionist', icon: '\u2726', name: 'المجموعة كاملة',
      how: 'شاهد كل القطع', test: p => p.seen.length >= CFG.items.length },
  ];
  ...
  function checkRoomDone(p, it) {
    ...
    if (bubble && bubbleText) {
      ...
      bubbleText.textContent = it.artist
        ? 'هذا كل ما أنجزه ' + it.artist.split(' ')[0] + ' حتى الآن.'
        : 'هذه كل القطع التي لدينا من ' + it.place + '. ' + (CFG.items.length - p.seen.length)
  ...
  if (btShare) btShare.addEventListener('click', async () => {
    ...
    openShare({
      title: b ? b.name : 'حصلت على شارة',
      caption: 'حصلت على شارة "' + (b ? b.name : 'explorer') + '" في '
        + 'مجموعة Tanit XR: ' + p.seen.length + ' من ' + CFG.items.length + ' قطعة من التراث التونسي، '
        + 'كلها مسحها متطوعون بهواتفهم.',
      link: pageLink(),
      picture: pic,
    });
  });
  ...
  function showCameo(it) {
    ...
    cameoTimer = setTimeout(() => {
      ...
      if (roomBtn) {
        ...
        roomBtn.hidden = n < 2;
        roomBtn.textContent = 'شاهد كل الـ' + n + ' في قاعة واحدة';
        roomBtn.onclick = () => {
          cameo.hidden = true;
  ...
  function paintInside(it) {
    ...
    insideBtn.textContent = inside ? 'اخرج' : 'ادخل';
  }
  ...
  if (shareBtn) shareBtn.addEventListener('click', async () => {
    ...
    openShare({
      ...
      caption: s0.it.real
        ? s0.it.title + '، مسحها متطوعو Tanit XR في تونس حتى لا تضيع أبدًا. شاركها لتساعد في حمايتها.'
        : s0.it.title + '، صمّمها ' + (s0.it.artist || 'متطوع من Tanit XR') + ' لمتحف Tanit XR الافتراضي. '
  ...
  function maybeOfferDemo() {
    ...
    bubble.hidden = false;
    if (dot) dot.classList.remove('in');
    if (bubbleText) bubbleText.textContent = 'تريد أن ترى كيف نصنع هذه؟';
    if (bubbleLong) bubbleLong.hidden = true;
    if (moreBtn) {
      moreBtn.textContent = 'أريني';
  ...
  const REACTIONS = [
    'انظر الآن إلى الخلف. دار متطوع حولها كلها بهاتف.',
    'تمهّل عند التفاصيل. الملمس هو صور الهاتف نفسها، مخيطة معًا.',
    'كل جانب حقيقي. لا شيء هنا اختُرع لسدّ فراغ.',
  ];
  let reactTimer = null;
  function maybeReact() {
  ...
  function answer(q) {
    ...
    if (has(/\b(size|big|tall|high|height|wide|long|measure|taille|grand|haut|large|mesure|حجم|طول|ارتفاع|كبير|عرض)\b/))
      return it.size ? it.title + ': ' + it.size + '. ' + 'مقاسة من المسح نفسه، لا تخمينًا.'
                     : 'صُمّمت يدويًا، فلا مقاس حقيقي لها. في المتحف تقف على ارتفاع مريح.';
    if (has(/\b(where|place|site|found|from|location|où|lieu|endroit|trouvé|أين|مكان|موقع|من أين)\b/))
      return it.place + (it.gps ? '. ' + 'الموقع الدقيق على شريحة الخريطة تحت البطاقة.' : '.');
    if (has(/\b(who|scan|scanned|made|model|modelled|volunteer|qui|numéris|bénévole|من|مسح|صنع|متطوع)\b/))
      return it.credit || 'متطوع من Tanit XR.';
    if (has(/\b(when|old|age|date|century|year|period|era|quand|ancien|siècle|époque|متى|قديم|قرن|تاريخ|عصر)\b/))
      return /\d{3,4}|century|siècle|قرن|BCE|CE\b/.test(it.note || '') ? it.note
           : 'البطاقة لا تذكر تاريخًا لهذه. صفحة الأرشيف فيها الوصف الكامل.';
    if (has(/\b(material|stone|marble|wood|clay|bronze|made of|matière|pierre|marbre|bois|argile|مادة|حجر|رخام|خشب|طين)\b/))
      return /marble|stone|wood|clay|bronze|plaster|tile|marbre|pierre|bois|argile|رخام|حجر|خشب|طين|جص/i.test(it.note || '') ? it.note
           : 'البطاقة لا تذكر المادة. أدِرها وانظر إلى السطح: المسح يحفظ كل حبيبة.';
    if (has(/\b(share|send|friend|partag|envoyer|ami|شارك|أرسل|صديق)\b/)) {
      ...
      return 'أفتح لك بطاقة المشاركة.';
    }
    if (has(/\b(love|save|keep|garder|enregistr|aimer|حفظ|أحب)\b/)) {
      if (uiSave) setTimeout(() => uiSave.click(), 300);
      return 'حُفظت. هي في مجموعتك، أعلى اليسار.';
    }
    if (has(/\b(vr|headset|casque|ar|quest|نظارة|افتراضي)\b/))
      return document.getElementById('vr-button') ? 'اضغط شاهدها بالواقع الافتراضي أسفل الصفحة.'
           : 'على النظارة، في هذه الصفحة زر شاهدها بالواقع الافتراضي. على الهاتف، زر شارك يعطيك صورة تحتفظ بها.';
    if (has(/\b(help|donate|give|support|money|don|soutenir|aider|تبرع|دعم|مساعدة)\b/))
      return 'كل ما هنا مجاني ومن صنع متطوعين. زر تبرّع تحت البطاقة يبقيه كذلك.';
    return (it.note ? it.note + ' ' : '') + 'هذا كل ما تعرفه البطاقة. صفحة الأرشيف فيها الوصف الكامل، وربما يعرف المتطوع الذي صنعها أكثر.';
  }
  if (askBox) {
  ...
  const NUDGES = [
    { id: 'friend', when: () => friendThumbs() !== '', btn: null, html: friendThumbs,
      line: () => 'أيّها أُري صديقك؟ انقر عليها وسأجهّز البطاقة.',
      act: () => {} },
    { id: 'share', when: () => true, btn: 'شاركها',
      line: () => 'تعرف شخصًا سيحب هذه القطعة؟ شاركها. كل مشاركة تبقيها حيّة.',
      act: () => { const b = document.getElementById('wf-share'); if (b) b.click(); } },
    { id: 'donate', when: () => true, btn: 'تبرّع',
      line: () => 'كل ما هنا مجاني ومن صنع متطوعين. إن كان يعني لك شيئًا، فتبرّع صغير يبقيه مستمرًا.',
      act: () => window.open(CFG.links.donate, '_blank', 'noopener') },
    { id: 'gallery', when: it => !!it.artist && CFG.items.filter(x => x.artist === it.artist).length > 2, btn: 'زُر المعرض',
      line: it => 'هذه واحدة من ' + CFG.items.filter(x => x.artist === it.artist).length + ' قطع صنعها ' + it.artist.split(' ')[0] + '. تريد رؤيتها كلها معًا؟',
      act: it => openRoom(it.artist) },
    { id: 'volunteer', when: () => true, btn: 'انضم إلينا',
      line: () => 'نفعل هذا كل خميس، معًا، من أربع قارات. يمكنك أن تكون واحدًا منا.',
      act: () => { location.href = linkUrl(CFG.links.volunteer); } },
    { id: 'newsletter', when: () => true, btn: 'اشترك',
      line: () => 'كل أسبوعين نرسل منحًا ودعوات مفتوحة للفنانين وصنّاع الواقع الممتد. تريدها في بريدك؟',
      act: () => { location.href = linkUrl(CFG.links.newsletter); } },
    { id: 'save', when: () => readSaved().length === 0, btn: 'أحبب هذه',
      line: () => 'إن أعجبتك إحداها، اضغط على القلب. تتجمّع كلها في مكان واحد لك.',
      act: () => { if (uiSave) uiSave.click(); } },
    { id: 'vr', when: () => !!document.getElementById('vr-button'), btn: 'شاهدها بالواقع الافتراضي',
      line: () => 'يمكنك الوقوف بجانب هذه القطعة في نظارتك.',
  ...
  const TOUR = [
    { pick: it => it.slug.startsWith('tanit-stela'),
      line: 'مرحبًا. أنا نورا. دعني أريك ما بناه مئة متطوع بهواتفهم. وُضع هذا الحجر في قرطاج قبل أكثر من ألفي عام. ومسحه متطوع في دقائق.',
      btn: 'كيف تمسح حجرًا؟', act: 'demo' },
    { pick: it => it.artist === 'Rachel West',
      line: 'ليس كل ما هنا مسحًا. راشيل لم تزر تونس قط. تعلّمت تاريخها في مكالمات الخميس وصمّمت هذه القطعة لمتحفنا. لكل متطوع معرض خاص به.',
      btn: "Visit Rachel’s gallery", act: 'room' },
    { pick: it => it.slug.includes('neapolis'),
      line: 'في جانفي كشفت عاصفة هذه المدينة القديمة لأيام قليلة. وصل متطوع إلى الشاطئ ومسحها قبل أن يستعيدها البحر. هذا هو السجل ثلاثي الأبعاد الوحيد الموجود. لهذا نسارع.',
      btn: 'التالي', act: 'next' },
    { pick: it => it.slug === 'patrick-museum-main-hall',
      line: 'يبني المتطوعون متحفًا كاملًا لهذه القطع، قاعة بعد قاعة، في وقت فراغهم. باتريك صمّم هذه القاعة، وآخرون يؤثثونها.',
      btn: 'التالي', act: 'next' },
    { pick: it => it.slug.startsWith('tanit-stela'),
      line: (CFG.stats ? CFG.stats.scans + ' مسحًا، ' + CFG.stats.sites + ' مواقع، ' + CFG.stats.volunteers
              + ' متطوعًا في أربع قارات، ولا شخص واحد مدفوع الأجر. '
            : 'مسوحات ومواقع ومتطوعون في أربع قارات، ولا شخص واحد مدفوع الأجر. ')
            + 'كل ما رأيته مجاني، إلى الأبد. هذا ما يفعله دعمك.',
      btn: null, act: 'end' },
  ];
    ...
    if (bubbleLong) {
      if (step.act === 'end') {
        bubbleLong.innerHTML = '<span class="tour-ends">'
          + '<a class="wf-btn gold" href="' + CFG.links.donate + '" target="_blank" rel="noopener">تبرّع</a>'
          + '<a class="wf-btn" href="' + linkUrl(CFG.links.volunteer) + '">تطوّع</a>'
          + '<a class="wf-btn" href="' + linkUrl(CFG.links.newsletter) + '">اشترك</a></span>';
        bubbleLong.hidden = false;
  ...
  if (shareColl) shareColl.addEventListener('click', async () => {
    ...
    openShare({
      title: 'مجموعتي',
      caption: 'مجموعتي من التراث التونسي الذي مسحه متطوعو Tanit XR، '
        + list.length + ' قطع. شاركها لتساعد في حمايتها.',
  ...
  function makePoster(badge) {
    ...
    x.textAlign = 'left';
    const p = readProg();
    const eyebrow = badge ? 'شارة جديدة'
      : [s.it.place, s.it.size].filter(Boolean).join('  \u00b7  ').toUpperCase();
    const heading = badge ? badge.name : s.it.title;
    const under = badge
      ? p.seen.length + ' من ' + CFG.items.length + ' قطعة شوهدت في مجموعة Tanit XR'
      : (s.it.credit || 'مسحها متطوع من Tanit XR');
  ...
  async function openShare({ title, caption, link, picture }) {
    if (!sheet) return;
    ssTitle.textContent = title || 'شارك';
    ...
    Object.keys(opens).forEach(net => {
      ...
      a2.onclick = async () => {
        ...
        if (net === 'linkedin' || net === 'facebook') {
          ...
          hint(ok ? 'نُسخ التعليق. ألصقه في منشورك عندما يُفتح.' : 'ألصق التعليق من المربع أعلاه في منشورك.');
        } else hint('جارٍ فتح ' + a2.textContent.trim() + ' مع التعليق جاهزًا.');
        bump(p => { p.shared++; });
    ...
    if (igBtn) igBtn.onclick = async () => {
      ...
      const ok = await copy(text() + '\n' + link);
      hint((saved ? 'حُفظت الصورة' : 'احفظ الصورة') + (ok ? ' ونُسخ التعليق. ' : '. ')
    ...
    if (saveBtn) {
      ...
      saveBtn.onclick = () => { saveImage(); hint('حُفظت الصورة في التنزيلات.'); };
    }
    sheet.hidden = false;
  ...
  function makeLabel(it, saved) {
    ...
    if (saved !== undefined) {
      ...
      x.fillText(saved ? '♥  محفوظة' : 'وجّه هنا واضغط الزناد للحفظ', W - 44, H - 40);
      x.textAlign = 'left';
    }
  ...
  function xrMenuItems() {
    const items = [];
    if (roomMode) items.push({ label: 'العودة إلى المجموعة', act: () => closeRoom() });
    const firstReal = slots.findIndex(s => s.it.real), firstMade = slots.findIndex(s => !s.it.real);
    if (firstReal >= 0) items.push({ label: 'مُسحت في تونس', act: () => { if (roomMode) closeRoom(); xrTarget = xrCursor = firstReal; } });
    if (firstMade >= 0) items.push({ label: 'من إنجاز المتطوّعين', act: () => { if (roomMode) closeRoom(); xrTarget = xrCursor = firstMade; } });
    ...
    if (saved.length) items.push({ label: 'قطعك المحفوظة (' + saved.length + ')',
      act: () => openRoom('مجموعتك', { hall: arMode, plain: true, list: slots.filter(s => saved.includes(s.it.slug)) }) });
    items.push({ label: arMode ? 'الخروج من الوضع المختلط' : 'الخروج من الواقع الافتراضي', act: () => { const ss = renderer.xr.getSession(); if (ss) ss.end(); } });
    return items;
  }
  ...
  if (navigator.xr && navigator.xr.isSessionSupported) {
    navigator.xr.isSessionSupported('immersive-vr').then(ok => {
      ...
      return import('three/addons/webxr/VRButton.js').then(mod => {
        ...
        btn.textContent = 'شاهدها بالواقع الافتراضي';
        document.body.appendChild(btn);
        renderer.xr.addEventListener('sessionstart', () => { btn.textContent = arMode ? 'شاهدها بالواقع الافتراضي' : 'الخروج من الواقع الافتراضي'; });
        renderer.xr.addEventListener('sessionend', () => { btn.textContent = 'شاهدها بالواقع الافتراضي'; });
      });
  ...
  function layoutRoom(list, opts) {
    ...
    signAt.forEach(({ s, isMade }) => {
      ...
      const sg = plain ? roomSign(who, cnt + (cnt === 1 ? ' قطعة' : ' قطع') + ' حفظتها')
        : roomSign(isMade ? 'صمّمها ' + first : 'حسّنها ' + first,
        cnt + (isMade ? (cnt === 1 ? ' قطعة' : ' قطع') + ' صُنعت من الصفر' : (cnt === 1 ? ' مسح' : ' مسوحات') + ' جُهّزت للويب'));
  ...
  function paintRoomBar() {
    ...
    if (roomEyebrow) roomEyebrow.textContent = roomFocus ? 'عن قرب' : 'معرض';
    if (roomCount) {
      const bits = [];
      if (roomMode.made) bits.push(roomMode.made + ' مصمّمة');
      if (roomMode.opt) bits.push(roomMode.opt + ' محسّنة');
  ...
  if (/[?&]hall=1/.test(location.search)) {
    ...
    setTimeout(() => {
      openRoom('داخل قاعة المتحف، معاينة', { hall: true, list: hallList });
  ...
  setTimeout(() => {
    ...
    if (bubbleText) bubbleText.textContent = prog.seen.length > 3 && left > 0
      ? 'أهلًا بعودتك! ' + prog.seen.length + ' شاهدتها حتى الآن، ' + left + ' متبقية.'
      : 'مرحبًا، أنا نورا. اسحب أي قطعة لتديرها، واحفظ ما يعجبك، وانظر كم '
        + 'شارة يمكنك جمعها في الطريق.';
    if (bubbleLong) bubbleLong.hidden = true;
    if (moreBtn) { moreBtn.hidden = true; moreBtn.dataset.offer = ''; }
    beHappy();
    flare = 1;
