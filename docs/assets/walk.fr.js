...
function start() {
  ...
  function openBubble() {
    ...
    if (speakBtn) speakBtn.title = s.it.voice ? 'Écouter Nura' : 'Lire à voix haute';
    ...
    if (moreBtn) {
      moreBtn.textContent = 'Dis-m’en plus';
  ...
  if (moreBtn) moreBtn.addEventListener('click', () => {
    ...
    moreBtn.textContent = expanded ? 'Merci, Nura' : 'Dis-m’en plus';
  ...
  function paintStats(it) {
    ...
    counter('collection_view/' + it.slug).then((v) => {
      ...
      statsEl.textContent = m ? 'vu ' + m.toLocaleString() + '+ fois' : '';
      statsEl.hidden = !m;
    });
  }
  ...
  function paintMore() {
    ...
    if (snd) snd.textContent = soundOn ? 'Son : activé' : 'Son : coupé';
  ...
  function paintSound() {
    ...
    sndBtn.title = (soundOn ? 'Le son est activé. Touchez pour couper.' : 'Le son est coupé. Touchez pour l’activer.') + cr;
  }
  paintSound();
  ...
  const BADGES = [
    { id: 'first-turn', icon: '\u21bb', name: 'Premier tour',
      how: 'Faites tourner un objet avec le curseur', test: p => p.rotated >= 1 },
    { id: 'curator', icon: '\u2665', name: 'Conservateur',
      how: 'Aimez cinq objets', test: p => p.saved.length >= 5 },
    { id: 'listener', icon: '\u25cf', name: 'Bonne oreille',
      how: 'Demandez à Nura d’en dire plus sur dix objets', test: p => p.more >= 10 },
    { id: 'surveyor', icon: '\u25c8', name: 'Arpenteur des sites',
      how: 'Voyez un objet de chaque lieu que nous avons numérisé',
      test: p => PLACES.every(pl => p.places.includes(pl)) },
    { id: 'guardian', icon: '\u2691', name: 'Gardien',
      how: 'Partagez un objet pour que d’autres le voient', test: p => p.shared >= 1 },
    { id: 'completionist', icon: '\u2726', name: 'Collection complète',
      how: 'Regardez chaque objet', test: p => p.seen.length >= CFG.items.length },
  ];
  ...
  function checkRoomDone(p, it) {
    ...
    if (bubble && bubbleText) {
      ...
      bubbleText.textContent = it.artist
        ? 'Voilà tout ce que ' + it.artist.split(' ')[0] + ' a réalisé jusqu’ici.'
        : 'Voilà toutes les pièces que nous avons de ' + it.place + '. ' + (CFG.items.length - p.seen.length)
          + ' restantes dans toute la collection.';
  ...
  if (btShare) btShare.addEventListener('click', async () => {
    ...
    openShare({
      title: b ? b.name : 'Badge obtenu',
      caption: 'J’ai obtenu le badge « ' + (b ? b.name : 'explorer') + ' » dans la '
        + 'collection Tanit XR : ' + p.seen.length + ' sur ' + CFG.items.length + ' objets du patrimoine tunisien, '
        + 'tous numérisés par des bénévoles avec leur téléphone.',
      link: pageLink(),
      picture: pic,
    });
  });
  ...
  function showCameo(it) {
    ...
    cameoTimer = setTimeout(() => {
      ...
      cameoLine.textContent = 'C’est moi qui l’ai ' + p.verb + '.';
      ...
      if (roomBtn) {
        ...
        roomBtn.hidden = n < 2;
        roomBtn.textContent = 'Voir les ' + n + ' ensemble dans une salle';
        roomBtn.onclick = () => {
          cameo.hidden = true;
  ...
  function paintInside(it) {
    ...
    insideBtn.textContent = inside ? 'Ressortir' : 'Entrer';
  }
  ...
  function paintSave(it) {
    ...
    uiSave.textContent = on ? 'Aimé ♥' : 'Aimer ♡';
  }
  ...
  if (shareBtn) shareBtn.addEventListener('click', async () => {
    ...
    openShare({
      title: 'Partagez pour le protéger',
      // a scan and a hand-made piece deserve different words
      caption: s0.it.real
        ? s0.it.title + ', numérisé en Tunisie par les bénévoles de Tanit XR pour qu’il ne soit jamais perdu. Partagez-le pour aider à le protéger.'
        : s0.it.title + ', modélisé par ' + (s0.it.artist || 'un bénévole de Tanit XR') + ' pour le musée virtuel de Tanit XR. '
  ...
  function maybeOfferDemo() {
    ...
    bubble.hidden = false;
    if (dot) dot.classList.remove('in');
    if (bubbleText) bubbleText.textContent = 'Vous voulez voir comment nous les fabriquons ?';
    if (bubbleLong) bubbleLong.hidden = true;
    if (moreBtn) {
      moreBtn.textContent = 'Montre-moi';
  ...
  const NUDGES = [
    ...
    { id: 'donate', when: () => true, btn: 'Faire un don',
      line: () => 'Tout ici est gratuit et fait par des bénévoles. Si cela compte pour vous, un petit don permet de continuer.',
      act: () => window.open(CFG.links.donate, '_blank', 'noopener') },
    { id: 'gallery', when: it => !!it.artist && CFG.items.filter(x => x.artist === it.artist).length > 2, btn: 'Visiter la galerie',
      line: it => 'C’est l’une des ' + CFG.items.filter(x => x.artist === it.artist).length + ' pièces que ' + it.artist.split(' ')[0] + ' a réalisées. Vous voulez les voir toutes ensemble ?',
      act: it => openRoom(it.artist) },
    { id: 'volunteer', when: () => true, btn: 'Rejoignez-nous',
      line: () => 'Nous faisons cela chaque jeudi, ensemble, depuis quatre continents. Vous pourriez être des nôtres.',
      act: () => { location.href = linkUrl(CFG.links.volunteer); } },
    { id: 'newsletter', when: () => true, btn: 'S’abonner',
      line: () => 'Toutes les deux semaines, nous envoyons des bourses et des appels à candidatures pour artistes et créateurs XR. Vous les voulez dans votre boîte mail ?',
      act: () => { location.href = linkUrl(CFG.links.newsletter); } },
    { id: 'save', when: () => readSaved().length === 0, btn: 'Aimer celui-ci',
      line: () => 'Si l’un d’eux vous touche, appuyez sur Aimer. Ils se rassemblent au même endroit pour vous.',
      act: () => { if (uiSave) uiSave.click(); } },
    { id: 'vr', when: () => !!document.getElementById('vr-button'), btn: 'Le voir en VR',
      line: () => 'Vous pouvez vous tenir à côté de celui-ci dans votre casque.',
  ...
  const TOUR = [
    { pick: it => it.slug.startsWith('tanit-stela'),
      line: 'Bienvenue. Je suis Nura. Laissez-moi vous montrer ce qu’une centaine de bénévoles ont construit avec leur téléphone. Cette pierre a été posée à Carthage il y a plus de deux mille ans. Un bénévole l’a numérisée en quelques minutes.',
      btn: 'Comment numérise-t-on une pierre ?', act: 'demo' },
    { pick: it => it.artist === 'Rachel West',
      line: 'Tout ici n’est pas une numérisation. Rachel n’est jamais allée en Tunisie. Elle a appris son histoire lors de nos appels du jeudi et a modélisé ceci pour notre musée. Chaque bénévole a sa propre galerie.',
      btn: "Visit Rachel’s gallery", act: 'room' },
    { pick: it => it.slug.includes('neapolis'),
      line: 'En janvier, une tempête a mis au jour cette cité antique pendant quelques jours. Un bénévole a rejoint la plage et l’a numérisée avant que la mer ne la reprenne. C’est le seul relevé 3D qui existe. Voilà pourquoi nous nous pressons.',
      btn: 'Suivant', act: 'next' },
    { pick: it => it.slug === 'patrick-museum-main-hall',
      line: 'Les bénévoles construisent un musée entier pour ces objets, salle après salle, sur leur temps libre. Patrick a modélisé cette salle, d’autres la meublent.',
      btn: 'Suivant', act: 'next' },
    { pick: it => it.slug.startsWith('tanit-stela'),
      line: (CFG.stats ? CFG.stats.scans + ' numérisations, ' + CFG.stats.sites + ' sites, ' + CFG.stats.volunteers
              + ' bénévoles sur quatre continents, et pas une seule personne payée. '
            : 'Des numérisations, des sites, des bénévoles sur quatre continents, et pas une seule personne payée. ')
            + 'Tout ce que vous avez vu est gratuit, pour toujours. Voilà ce que fait votre soutien.',
      btn: null, act: 'end' },
  ];
    ...
    if (bubbleLong) {
      if (step.act === 'end') {
        bubbleLong.innerHTML = '<span class="tour-ends">'
          + '<a class="wf-btn gold" href="' + CFG.links.donate + '" target="_blank" rel="noopener">Faire un don</a>'
          + '<a class="wf-btn" href="' + linkUrl(CFG.links.volunteer) + '">Devenir bénévole</a>'
          + '<a class="wf-btn" href="' + linkUrl(CFG.links.newsletter) + '">S’abonner</a></span>';
  ...
  if (shareColl) shareColl.addEventListener('click', async () => {
    ...
    openShare({
      title: 'Ma collection',
      caption: 'Ma collection de patrimoine tunisien numérisé par les bénévoles de Tanit XR, '
        + list.length + ' objets. Partagez-les pour aider à les protéger.',
  ...
  function makePoster(badge) {
    ...
    x.textAlign = 'left';
    const p = readProg();
    const eyebrow = badge ? 'BADGE OBTENU'
      : [s.it.place, s.it.size].filter(Boolean).join('  \u00b7  ').toUpperCase();
    const heading = badge ? badge.name : s.it.title;
    const under = badge
      ? p.seen.length + ' sur ' + CFG.items.length + ' objets vus dans la collection Tanit XR'
      : (s.it.credit || 'Numérisé par un bénévole de Tanit XR');
  ...
  async function openShare({ title, caption, link, picture }) {
    if (!sheet) return;
    ssTitle.textContent = title || 'Partager';
    ...
    Object.keys(opens).forEach(net => {
      ...
      a2.onclick = async () => {
        ...
        if (net === 'linkedin' || net === 'facebook') {
          ...
          hint(ok ? 'Légende copiée. Collez-la dans votre publication quand elle s’ouvre.' : 'Collez la légende ci-dessus dans votre publication.');
        } else hint('Ouverture de ' + a2.textContent.trim() + ' avec la légende pré-remplie.');
        bump(p => { p.shared++; });
    ...
    if (igBtn) igBtn.onclick = async () => {
      ...
      const ok = await copy(text() + '\n' + link);
      hint((saved ? 'Image enregistrée' : 'Enregistrez l’image') + (ok ? ' et légende copiée. ' : '. ')
    ...
    if (copyBtn) copyBtn.onclick = async () => {
      ...
      hint(ok ? 'Lien copié.' : link);
    };
    const saveBtn = el('save');
    if (saveBtn) {
      saveBtn.hidden = !(picture && picture.blob);
      saveBtn.onclick = () => { saveImage(); hint('Image enregistrée dans vos téléchargements.'); };
    }
    sheet.hidden = false;
  ...
  function makeLabel(it, saved) {
    ...
    if (saved !== undefined) {
      ...
      x.fillText(saved ? '♥  ENREGISTRÉ' : 'VISEZ ICI ET PRESSEZ LA GÂCHETTE POUR ENREGISTRER', W - 44, H - 40);
      x.textAlign = 'left';
    }
  ...
  function xrMenuItems() {
    const items = [];
    if (roomMode) items.push({ label: 'Retour à la collection', act: () => closeRoom() });
    const firstReal = slots.findIndex(s => s.it.real), firstMade = slots.findIndex(s => !s.it.real);
    if (firstReal >= 0) items.push({ label: 'Numérisé en Tunisie', act: () => { if (roomMode) closeRoom(); xrTarget = xrCursor = firstReal; } });
    if (firstMade >= 0) items.push({ label: 'Réalisé par des bénévoles', act: () => { if (roomMode) closeRoom(); xrTarget = xrCursor = firstMade; } });
    ...
    if (saved.length) items.push({ label: 'Vos objets enregistrés (' + saved.length + ')',
      act: () => openRoom('Votre collection', { hall: arMode, plain: true, list: slots.filter(s => saved.includes(s.it.slug)) }) });
    items.push({ label: arMode ? 'Quitter la vue mixte' : 'Quitter la VR', act: () => { const ss = renderer.xr.getSession(); if (ss) ss.end(); } });
    return items;
  }
  ...
  function toggleXRMenu() {
    ...
    x.fillText('O Ù   A L L E R', 48, 66);
    xrMenuRows = [];
    items.forEach((it, k) => {
      const y0 = 96 + k * RH;
  ...
  if (navigator.xr && navigator.xr.isSessionSupported) {
    navigator.xr.isSessionSupported('immersive-vr').then(ok => {
      ...
      return import('three/addons/webxr/VRButton.js').then(mod => {
        ...
        btn.textContent = 'Le voir en VR';
        document.body.appendChild(btn);
        renderer.xr.addEventListener('sessionstart', () => { btn.textContent = arMode ? 'Le voir en VR' : 'Quitter la VR'; });
        renderer.xr.addEventListener('sessionend', () => { btn.textContent = 'Le voir en VR'; });
      });
  ...
  function layoutRoom(list, opts) {
    ...
    signAt.forEach(({ s, isMade }) => {
      ...
      const sg = plain ? roomSign(who, cnt + (cnt === 1 ? ' objet' : ' objets') + ' enregistrés')
        : roomSign(isMade ? 'Modélisé par ' + first : 'Optimisé par ' + first,
        cnt + (isMade ? (cnt === 1 ? ' pièce' : ' pièces') + ' créées de zéro' : (cnt === 1 ? ' numérisation' : ' numérisations') + ' préparées pour le web'));
  ...
  function paintRoomBar() {
    ...
    if (roomEyebrow) roomEyebrow.textContent = roomFocus ? 'De près' : 'Galerie';
    if (roomCount) {
      const bits = [];
      if (roomMode.made) bits.push(roomMode.made + ' modélisés');
      if (roomMode.opt) bits.push(roomMode.opt + ' optimisés');
  ...
  if (/[?&]hall=1/.test(location.search)) {
    ...
    setTimeout(() => {
      openRoom('Dans la salle du musée, un aperçu', { hall: true, list: hallList });
